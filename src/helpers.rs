...
pub fn query_nft_balance(
    deps: Deps,
    cw721_contracts: Vec<Addr>,
    owner: Addr,
) -> StdResult<f64> {
    ...
    let total_entries = calculate_entries(total_nft_count) as f64; // Convert to f64
...
pub fn is_round_over(state: &LotteryState, current_time: &Timestamp) -> bool {
    if let Some(current_round) = state.rounds.last() {
        ...
        let round_end_time = current_round.round_start_time.plus_seconds(state.round_duration);
