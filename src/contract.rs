...
use cosmwasm_std::{
    ensure_eq, to_json_binary, Addr, BankMsg, Binary, Coin, Deps, DepsMut, Env, MessageInfo,
...
use crate::helpers::{get_balance, is_round_over, query_nft_balance, validate_lottery_active};
use crate::msg::{ExecuteMsg, InstantiateMsg, JoinLotteryMsg, QueryMsg};
use crate::state::NOIS_PROXY;
use crate::state::{LotteryState, Round, LOTTERY_STATE};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateCooldownPeriod(cooldown_period) => {
            execute_update_cooldown_period(deps, _env, info, cooldown_period)
        }
        ExecuteMsg::UpdateEntries(entries) => execute_update_entries(deps, _env, info, entries),
        ExecuteMsg::UpdateEntryFee(entry_fee) => {
            execute_update_entry_fee(deps, _env, info, entry_fee)
        }
        ExecuteMsg::UpdateOwner(owner) => execute_update_owner(deps, _env, info, owner),
        ExecuteMsg::UpdatePauseStatus(pause_status) => {
            execute_update_pause_status(deps, _env, info, pause_status)
        }
        ExecuteMsg::UpdateRoundDuration(round_duration) => {
            execute_update_round_duration(deps, _env, info, round_duration)
        }
...
pub fn execute_join_lottery(
    ...
    let total_entries: f64 = query_nft_balance(
        deps.as_ref(),
        vec![
            Addr::unchecked("inj12lq764spe67c47lg29vcfqe7kdd9fmguxck05l"), // Addr::unchecked("inj1vtd54v4jm50etkjepgtnd7lykr79yvvah8gdgw"),
    ...
    if let Some(current_round) = rounds.last_mut() {
        ...
        let entry_fee_per_ticket = lottery_state.entry_fee;
...
pub fn execute_receive(
    ...
    let winner_index = ints_in_range(
        randomness,
        1,
        0,
        current_round.participants.len() as u32 - 1,
    )[0] as usize;
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> Result<Binary, StdError> {
    match msg {
        ...
        QueryMsg::RoundInfo(round_info) => {
            to_json_binary(&query_round_info(deps, env, round_info.round_number)?)
        }
    }
}
